...
if dim ==3  %compute: H(X), H(Y), H(Z),
    ...
    I_x1y=0; I_x2y=0; I_x1x2=0; T1 = 0; T2=0; Itot_test=0;
    ...
    for i=1:N
        for j=1:N
            for k=1:N
                ...
                %conditional info = U_i + S
                if m_ijk > eps && m_ij(i,j)> eps && m_jk(j,k) >eps && m_j(j)>eps
                    Cond1_add = m_ijk*log2((m_ijk*m_j(j))/(m_ij(i,j)*m_jk(j,k)));
                ...
                %conditional info: U_j +S
                if m_ijk > eps && m_ij(i,j)> eps && m_ik(i,k) >eps && m_i(i)>eps
                    Cond2_add = m_ijk*log2((m_ijk*m_i(i))/(m_ij(i,j)*m_ik(i,k)));
                ...
                I_tau4_add = m_ijk*log2(m_ijk/(m_ij(i,j)*m_k(k)));
                if I_tau4_add>0
                    Itot_test = Itot_test+I_tau4_add;
                end
    ...
    dI = T1-I_x1y; %interaction information = S-R (if positive: synergy dominates)
